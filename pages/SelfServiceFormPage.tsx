...
import { SparklesIcon, LoadingIcon, CameraIcon, AddIcon, DeleteIcon } from '../components/icons';
import { generateEmployeeData } from '../services/geminiService';
import { calculateSuccessionStatus, getBirthDateFromNIP } from '../utils/talentUtils';

interface SelfServiceFormPageProps {
    onSave: (employee: Employee) => void;
}

const SelfServiceFormPage: React.FC<SelfServiceFormPageProps> = ({ onSave }) => {
    const getInitialFormData = (): Omit<Employee, 'id' | 'avatar' | 'skills'> & { skills: string } => ({
        ...
        trainingAttended: '',
        eselon: 'Staf',
        performance: 75, // Default value, not shown to user
        potential: 75, // Default value, not shown to user
        competency: 75, // Default value, not shown to user
        skills: '',
        criticalPosition: '',
        developmentPlan: '',
        successionStatus: 'Bukan Kandidat', // Default value, not shown to user
        educationHistory: [],
        performanceHistory: [],
        careerHistory: [],
        developmentHistory: [],
    ...
    }, [formData.performance, formData.potential, formData.nip, formData.eselon]);
    ...
    const handleAddItem = (field: 'educationHistory' | 'performanceHistory' | 'careerHistory' | 'developmentHistory') => {
        ...
        switch (field) {
            case 'educationHistory': newItem = { id: crypto.randomUUID(), jenjang: '', jurusan: '', institusi: '', tahunLulus: '' }; break;
            case 'performanceHistory': newItem = { id: crypto.randomUUID(), tahun: '', skp: '', predikat: '' }; break;
            case 'careerHistory': newItem = { id: crypto.randomUUID(), jabatan: '', unitKerja: '', tmt: '' }; break;
            case 'developmentHistory': newItem = { id: crypto.randomUUID(), namaPelatihan: '', penyelenggara: '', tahun: '', jenis: 'Klasikal' }; break;
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newEmployeeData: Omit<Employee, 'id' | 'avatar'> = {
            ...
            pendidikan: latestEducation ? latestEducation.jenjang : formData.pendidikan,
        ...
        const newEmployee: Employee = {
            id: formData.nip, // Will be set in parent
            ...newEmployeeData,
    ...
    const handleCancel = () => {
        if (window.confirm("Apakah Anda yakin ingin mengosongkan semua isian pada formulir? Tindakan ini tidak dapat diurungkan.")) {
    ...
    const eselonOptions = [ 'Staf', 'Pelaksana', 'Fungsional Terampil', 'Fungsional Ahli Pertama', 'Fungsional Ahli Muda', 'Fungsional Ahli Madya', 'Fungsional Ahli Utama', 'Pengawas (Eselon IV)', 'Administrator (Eselon III)', 'JPT Pratama (Eselon II)', 'JPT Madya (Eselon I.b)', 'JPT Utama (Eselon I.a)'];
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg w-full max-w-5xl mx-auto relative">
             {isGenerating && (
            ...
            <form onSubmit={handleSubmit}>
                <div className="p-6 border-b border-gray-200 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        <h2 className="text-xl font-bold text-gray-900">Formulir Data Talenta ASN</h2>
                 ...
                 <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <div className="md:col-span-2 mt-4 space-y-3">
                        <div className={historySectionHeaderStyle}><h3 className={historySectionTitleStyle}>Data Pendidikan</h3><button type="button" onClick={() => handleAddItem('educationHistory')} className={addButtonStyle}><AddIcon className="h-4 w-4" /><span>Tambah</span></button></div>
                        {formData.educationHistory?.map((item) => (
                ...
                <div className="p-6 bg-gray-50 rounded-b-xl flex justify-end space-x-3">
                    <button type="button" onClick={handleCancel} className="px-5 py-2.5 bg-white border border-gray-300 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">Kosongkan Formulir</button>
                    <button type="submit" className="px-5 py-2.5 bg-indigo-600 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">Simpan & Daftarkan Data</button>
                </div>
            </form>
        </div>
    );
...
export default SelfServiceFormPage;
